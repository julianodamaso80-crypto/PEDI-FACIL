...
import { LayoutDashboard, Store, Receipt, LogOut } from "lucide-react"
    ...
    <div className="flex h-screen bg-gray-950 text-white">
      <aside className="w-64 bg-gray-950 border-r border-gray-800 flex flex-col">
        ...
        <div className="p-4 border-t border-gray-800">
          <p className="text-xs text-gray-500 mb-3 px-3">{session.user.email}</p>
          <form action="/api/auth/signout" method="POST">
            <button
              type="submit"
              className="flex w-full items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-400 hover:bg-gray-800 hover:text-red-400 transition-colors"
            >
